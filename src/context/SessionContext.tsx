...
import { formatSessionDate, formatSessionDayOfWeek, formatSessionTime, toPersianDigits } from '../utils/dateFormatter';
...
function mapMovieData(raw: any, questionsRaw?: any[], vocabRaw?: any[]): MovieData {
  const durationMin = raw.duration_minutes || raw.durationMinutes || (typeof raw.duration === 'number' ? raw.duration : null);
  const durationStr = durationMin ? `${durationMin} min` : (raw.duration || '90 min');
  const durationFaStr = durationMin ? `${toPersianDigits(durationMin)} دقیقه` : (raw.duration_fa || raw.durationFa || '۹۰ دقیقه');

  const rawGenreEn = raw.genre || raw.genre_en || raw.genres;
  const genreEnStr = Array.isArray(rawGenreEn) ? rawGenreEn.join(', ') : (rawGenreEn || 'Cinema');

  const rawGenreFa = raw.genre_fa || raw.genreFa;
  const genreFaStr = Array.isArray(rawGenreFa) ? rawGenreFa.join('، ') : (rawGenreFa || 'سینما');
  ...
  let vocabulary: { term: string; definitionFa: string; definitionEn: string; example: string }[] = [];
...
export const SessionProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const fetchCapacityStatus = useCallback(async (sessionId: string | number) => {
    try {
      setIsCapacityLoading(true);
      const { data, error: capErr } = await supabase.rpc('get_session_capacity_status', {
  ...
  const fetchActiveSessionAndMovie = useCallback(async () => {
    ...
    try {
      ...
      if (sessionData) {
        const rawDate = sessionData.session_date || sessionData.sessionDate || sessionData.date || '';
        const rawTime = sessionData.session_time || sessionData.sessionTime || sessionData.time || '17:00';

        const locationNameFa = sessionData.location_name_fa || sessionData.locationNameFa || 'کافه';
        const locationNameEn = sessionData.location_name_en || sessionData.locationNameEn || 'Cafe';
        const locationAddressFa = sessionData.location_address_fa || sessionData.locationAddressFa || 'تهران';
        const locationAddressEn = sessionData.location_address_en || sessionData.locationAddressEn || 'Tehran';
        ...
        const activeSession: ActiveSessionInfo = {
          ...
          capacity: sessionData.capacity !== undefined && sessionData.capacity !== null ? Number(sessionData.capacity) : 8,
          rawDate,
          rawTime,
          locationNameFa,
          locationNameEn,
          ...
          hostFa: sessionData.host_fa || sessionData.hostFa || 'برگزارکننده مووی کلاب',
          hostEn: sessionData.host_en || sessionData.hostEn || 'Movie Club Host',
          priceNoteFa: sessionData.price_note_fa || 'هزینه سفارش شخصی در کافه به عهده هر شرکت‌کننده است',
          priceNoteEn: sessionData.price_note_en || 'Personal cafe orders are covered individually',
          image: sessionData.image_url || sessionData.image || 'https://images.unsplash.com/photo-1517256064527-09c73fc73e38?auto=format&fit=crop&w=1200&q=80',
        };
        ...
        if (movieId) {
          ...
          if (movieErr) {
            ...
            if (questionsErr) {
              console.warn('Supabase discussion questions query note:', questionsErr.message);
            }
  ...
  const getDateDisplay = useCallback(
    (lang: 'fa' | 'en') => {
      if (!session || !session.rawDate) {
        return lang === 'fa' ? 'جلسه بعدی به‌زودی اعلام می‌شود' : 'Next gathering will be announced soon';
      }
